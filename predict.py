...
def predict(image_path, model, topk=5):
    ...
    with torch.no_grad():
        ...
        classes=[idx_to_class[idx] for idx in top_indx[0].tolist()]
...
top_prob, classes = predict(filepath, model, clas_num)

#converting prob tensor into numpy array and choosing the first element

topprob=top_prob.cpu()
toppro=topprob.numpy()
topp=toppro[0]

#Mapping classes to classnames
classnames=[]

for c in classes:
    
    classnames.append(cat_to_name[c])
       
#looping over the classes to print class names and their probabilities
for a in range(clas_num):
    
    print("No. {}/{}.. ".format(a+1, clas_num),
          "Class name: {} ".format(classnames[a]),
          "Probability: {} ".format(topp[a])
         )
